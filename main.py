...
from com_spider import JDCommentSpider  # 保留评论爬虫的引用

import config


def crawl_comments(ids_collection_dir, output_dir):
    """Crawl comments for the products in the specified directory.
    ...
    for filename in files_to_process:
        ...
        if os.path.isfile(file_path) and filename.endswith('.json'):
            try:
                ...
                output_files = [file.split('.')[0] for file in output_files if file.endswith('.csv')]
                ...
                for product_info in list(data.values())[0]:
                    ...
                    # Sleep for a random amount of time between 0.33 to 1 minutes
