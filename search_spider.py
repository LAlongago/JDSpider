...
def jd_search_spider(keywords, save_dir):
    """Perform web scraping on the JD website for the specified keywords, after manual login.
    ...
    try:
        ...
        # Open the JD login page
        url = "https://passport.jd.com/new/login.aspx"
        driver.get(url)

        # 手动登录，替换自动登录
        input("请在浏览器中完成登录，然后在这里按Enter继续...")

        # No need to perform login programmatically, as user will do it manually
        # perform_login(driver, username, password)  # 移除自动登录
        ...
        for keyword in keywords:
            ...
            WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.CSS_SELECTOR, "li.gl-item")))
            scroll_to_half(driver)
            ...
            for product in products:
                sku_value = product.get_attribute("data-sku")
                desc = product.find_element(By.CSS_SELECTOR, "div.p-name em").text
                product_info = {"sku": sku_value, "description": desc}
                product_list.append(product_info)

            logger.info(f"Web scraping for keyword '{keyword}' completed successfully.")
            keyword_filename = get_pinyin_initials(f"{keyword.replace(' ', '_')}")
            save_product_ids({keyword: product_list}, save_dir, keyword_filename)
...
if __name__ == "__main__":
    # Perform web scraping on the JD website for the specified keywords
    jd_search_spider(keywords=config.KEYWORDS_TO_SEARCH, save_dir=config.PRODUCT_ID_DIR)
